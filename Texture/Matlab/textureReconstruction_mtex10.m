function [orired_f,odfred_f,ero,odf,varargout]=...
    textureReconstruction_mtex10(ns,varargin)
%%
...
%           'grains')
% [ori,odf,e]=ODF_reduction_algo(n,'orientation',ori)
% [ori,odf,e]=ODF_reduction_algo(n,'orientation',ori,...
%           'kernel',psi)
% [ori,odf,e]=ODF_reduction_algo(n,'orientation',ori,...
%           'kernelShape','kappa')
...
if length(varargin)>2
    options = {'grainsMatFile','grains','kernel','kernelShape', 'odf_true'};
            ...
            if ~isempty(loc)
              ...
              'Optimum kernel estimated from mean orientations of grains')
               ...
               'Optimum kernel estimated from mean orientations of grains')
                    ...
                    case 'odf_true'
                        fprintf('ODF given as input. Will use that as true odf.\n')
                        psi = SO3DeLaValleePoussinKernel('halfwidth',...
                            0.08726646259971647); % script requires this for determining initial kappa 
                        odf = varargin{loc+1};
...
if flag==1
    ...
    disp(['Default initial kernel shape factor: ',num2str(5),' rad'])
end
...
if ~ flag_odf
    fprintf('No ODF given as input. Will create it from kernel and ebsd mock.\n')
...
ll=5*degree; %grid was 10 in Abhishek's version
hl = 55*degree;
...
kappa = psi.halfwidth; % initial kernel shape
%%
tic
for hw = ll:step:hl
% hw
%% part of code from mtex
S3G = equispacedSO3Grid(ori.CS,ori.SS,'resolution',(hw/2));
