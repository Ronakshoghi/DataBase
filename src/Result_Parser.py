...
"""
This function read all the output files in the result folder and write them in the database. 
"""

def results_reader (key, cp_code='abaqus'):
    """
    Function reads the result file produced by the CP simulation.
    In case of OpenPhase, the results are written in a results.txt file directly by the OpenPhase C++ file. With this
    function, the txt file is read and converted into the JSON format to store it in the Data_Base.json later.
    Parameters
    ----------
    key : str
    ...
    for result_file in results_files_name:
        if cp_code == 'abaqus':
            ...
            # In the following, the result.txt file generated by OpenPhase after the simulation finishes is read.
            ...
            if len(openphase_out.shape) == 1:
                openphase_out = openphase_out.reshape((1, len(openphase_out)))
            stresses_cauchy = openphase_out[:, 0:6] * 1e-6 # convert to MPa
            stresses_vm = openphase_out[:, -1] * 1e-6 # convert to MPa

            # Read strains and stresses
