...
def WriteInputFile(fileName):
       ...
       # Cut of the Formatting Parameters
              ...
              #print zeile
              if zeile[0] == '*End Part':
                     content.append(['*Include, input=LeftToRight.inp'])
                     content.append(['*Include, input=BottomToTop.inp'])
                     content.append(['*Include, input=FrontToRear.inp'])
                     content.append(['*Include, input=Edges.inp'])
                     content.append(['*Include, input=Corners.inp'])
                     content.append(['*End Part'])

              elif zeile[0] == '*End Assembly':
                     content.append(['*Include, input=VerticeSets.inp'])
